...
type Message struct {
	...
	TargetLabel     string
	Format          string // "" plain, "md" markdown
	Body            string
	CreatedAt       time.Time
}
...
func (s *Store) CreateMessage(m Message) (Message, error) {
	res, err := s.db.Exec(
		`INSERT INTO messages (channel_id, kind, author_kind, author_name, agent_run_id, origin_message_id, target_label, format, body)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		m.ChannelID, m.Kind, m.AuthorKind, m.AuthorName, nz(m.AgentRunID), nz(m.OriginMessageID), m.TargetLabel, m.Format, m.Body)
	if err != nil {
		return Message{}, err
	}
	id, _ := res.LastInsertId()
...
const msgCols = `id, channel_id, kind, author_kind, author_name,
	COALESCE(agent_run_id, 0), COALESCE(origin_message_id, 0), target_label, format, body, created_at`

func scanMessage(sc interface{ Scan(...any) error }) (Message, error) {
	var m Message
	var ts string
	err := sc.Scan(&m.ID, &m.ChannelID, &m.Kind, &m.AuthorKind, &m.AuthorName,
		&m.AgentRunID, &m.OriginMessageID, &m.TargetLabel, &m.Format, &m.Body, &ts)
