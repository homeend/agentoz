...
`, provider.ShellQuote(erbrusBin))
	...
	// Codex 0.153 asks the human before running `erbrus msg send` unless an
	// execution-policy rule allows it (seen live 2026-09-08, after the
	// network fix). Rules load only from $CODEX_HOME/rules/*.rules (or the
	// trusted repo's .codex/rules), never from a flag, so erbrus keeps one
	// file of its own there. Best effort: a failure here must not stop the
	// spawn, codex merely asks as before.
	if p, err := codexRulesPath(); err == nil {
		ensureCodexRules(p, erbrusBin)
	}
	return "-c " + provider.ShellQuote(fmt.Sprintf(`notify=["%s"]`, notifyScript)) +
		" -c sandbox_workspace_write.network_access=true", nil
}

// codexRulesPath is erbrus's execution-policy file in the codex home
// ($CODEX_HOME, default ~/.codex). One file, owned by erbrus.
func codexRulesPath() (string, error) {
	...
	return filepath.Join(home, "rules", "erbrus.rules"), nil
}

// codexRules renders the allow rules for erbrusBin: the absolute path the
// preamble tells agents to run, plus the bare name for when it is on PATH.
// Starlark string literals take Go-style escapes, so %q is safe.
func codexRules(erbrusBin string) string {
	return fmt.Sprintf(`# Generated by erbrus; rewritten when the erbrus binary path changes.
# Lets agents spawned by erbrus report to their channel without an
# approval prompt. Only "erbrus msg ..." is allowed, nothing else.
prefix_rule(
    pattern = [%q, "msg"],
    decision = "allow",
    justification = "erbrus msg send/read: the agent talking to its channel on localhost",
)
prefix_rule(
    pattern = ["erbrus", "msg"],
    decision = "allow",
    justification = "same, when erbrus is on PATH",
)
`, erbrusBin)
}

// ensureCodexRules writes the rules file when it is missing or differs.
// Returns whether it wrote. Codex reads rules at startup, so a change
// reaches only codex instances started afterwards.
func ensureCodexRules(path, erbrusBin string) (bool, error) {
	want := codexRules(erbrusBin)
